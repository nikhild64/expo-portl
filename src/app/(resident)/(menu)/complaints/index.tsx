...
export default function ComplaintsScreen() {
  ...
  return (
    <Screen scroll safe={false} refreshing={refreshing} onRefresh={refresh} contentContainerStyle={{ paddingTop: 12, paddingBottom: 96 }}>
      ...
      <Button label="Raise complaint" icon="add" onPress={() => router.push('/(resident)/(menu)/complaints/new')} />
          ...
          complaints.map((complaint, index) => (
            ...
            >
              <ComplaintCard
                complaint={complaint}
                onPress={() =>
                  router.push({ pathname: '/(resident)/(menu)/complaints/[id]', params: { id: complaint.id } })
                }
              />
            </Animated.View>
          ))
